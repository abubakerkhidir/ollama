...
func GPUDevices(ctx context.Context, runners []ml.FilteredRunnerDiscovery) []ml.DeviceInfo {
	...
	if !bootstrapped {
		...
		// On macOS, enumerate and log all Metal devices (including eGPUs such as
		// the AMD Radeon VII and RX 6800) before starting bootstrap discovery.
		// This allows users to identify the correct OLLAMA_METAL_DEVICE index.
		// logAndPrepareMetalEnvs is a no-op on non-Darwin platforms.
		logAndPrepareMetalEnvs()
		...
		for dir := range libDirs {
			...
			// Merge in any macOS Metal device overrides so the bootstrap runner
			// targets the correct GPU (e.g. an external AMD eGPU).
			// metalDeviceEnvs returns nil on non-Darwin platforms.
			extraEnvs := metalDeviceEnvs()
			...
			devices = append(devices, bootstrapDevicesWithMetalRetry(ctx1stPass, ctx, bootstrapTimeout, dirs, extraEnvs)...)
			cancel()
		}
...
func overrideWarnings() {
	...
	for _, k := range []string{
		...
		// Prefer the structured AsMap entry; fall back to a direct env lookup
		// for variables (e.g. GGML_METAL_DEVICE_INDEX) that are not in AsMap.
		var val string
		if e, found := m[k]; found {
		...
		if val != "" {
			anyFound = true
			slog.Warn("user overrode visible devices", k, val)
		}
	}
	if anyFound {
